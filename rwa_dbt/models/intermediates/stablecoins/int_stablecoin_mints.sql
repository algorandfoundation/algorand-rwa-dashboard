...
SELECT 
  ...
  END AS asset,
  SUM(t.amount) / 1e6 AS mints
FROM mainnet.txn AS t
JOIN mint_wallets mw ON t.snd_addr_id = mw.wallet_id
WHERE t.asset_id IN (31566704, 312769, 760037151, 672913181)
  AND t.rcv_addr_id NOT IN (SELECT wallet_id FROM mint_wallets)
