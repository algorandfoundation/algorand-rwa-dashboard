...
const Certificates = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const urls = {
          certificates: import.meta.env.VITE_CERTIFICATES_CERTIFICATES,
          addresses: import.meta.env.VITE_CERTIFICATES_ADDRESSES,
        };
            ...
            .map(row => {
              ...
              if (type === 'certificates') {
                ...
                return {
                  ...
                  cumulative: parseInt(cols[4], 10) || 0
                };
        ...
        const [certificatesData, addressesData] = await Promise.allSettled([
          parseCSV(urls.certificates, 'certificates'),
          parseCSV(urls.addresses, 'addresses'),
        ]);

        setData({
          certificates: certificatesData.status === 'fulfilled' ? certificatesData.value : [],
          addresses: addressesData.status === 'fulfilled' ? addressesData.value : [],
        });
      } catch (err) {
  ...
  const certificatesKpi = getKpiData(data.certificates, 'certificates');
  const addressesKpi = getKpiData(data.addresses, 'addresses');
  ...
  const kpis = [
    {
      ...
      value: `${formatCompactNumber(certificatesKpi.value)}`,
      delta: certificatesKpi.delta,
  ...
  const getChartConfig = () => {
    switch (activeChart) {
      case 'certificates':
        return {
          leftLabel: 'Monthly Certificates',
          rightLabel: 'Total Certificates',
          monthlyName: 'Monthly Certificates',
          cumulativeName: 'Total Certificates',
          valuePrefix: '',
          valueSuffix: ''
        };
      case 'addresses':
