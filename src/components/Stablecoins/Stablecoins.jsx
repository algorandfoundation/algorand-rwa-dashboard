...
const Stablecoins = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const urls = {
          market_cap: import.meta.env.VITE_STABLECOINS_MARKETCAP,
          addresses: import.meta.env.VITE_STABLECOINS_ADDRESSES,
          volume: import.meta.env.VITE_STABLECOINS_VOLUME,
          holders: import.meta.env.VITE_STABLECOINS_HOLDERS
        };
          ...
          return rows
            .map(row => {
              ...
              if (type === 'market_cap') {
                ...
                return {
                  date: cols[0],
                  usdc: parseInt(cols[1], 10) || 0,
                  usdt: parseInt(cols[2], 10) || 0,
                  xusd: parseInt(cols[3], 10) || 0,
                  gousd: parseInt(cols[4], 10) || 0,
                ...
                return {
                  date: cols[0],
                  usdc: parseFloat(cols[1]) || 0,
                  usdt: parseFloat(cols[2]) || 0,
                  xusd: parseFloat(cols[3]) || 0,
                  gousd: parseFloat(cols[4]) || 0,
                  total: parseFloat(cols[5]) || 0
                };
    ...
    switch (activeChart) {
      case 'market_cap':
        return {
          ...
          valuePrefix: '$',
          bars: [
            { key: 'usdc', name: 'USDC', color: assetColors.usdc },
            { key: 'usdt', name: 'USDT', color: assetColors.usdt },
            { key: 'xusd', name: 'xUSD', color: assetColors.xusd },
            { key: 'gousd', name: 'goUSD', color: assetColors.gousd }
          ]
        };
      case 'addresses':
        return {
